...
class execution_engine2:
    ...
    GIT_COMMIT_HASH = "75d8824c809f3156760a4650a601d3d18df075dd"
    ...
    def check_job(self, ctx, params):
        ...
        :returns: instance of type "JobState" (job_id - string - id of the
           ...
           off of. Not available on a retry_parent itself retry_saved_toggle
           - bool - Marked true when all retry steps/txns have completed
           ...
           child_jobs - array - Only parent container should have child job
           ids scheduler_type - str - scheduler, such as awe or condor
           scheduler_id - str - scheduler generated id scheduler_estimator_id
           - str - id for the job spawned for estimation terminated_code -
           int - internal reason why a job was terminated, one of: 0 - user
    ...
    def check_job_batch(self, ctx, params):
        ...
        :returns: instance of type "CheckJobBatchResults" (parent_job - state
           ...
           the run_job call)  ## TODO - verify job_output - object - outputs
           from the job (from the run_job call) ## TODO - verify updated -
           int - timestamp since epoch in milliseconds of the last time the
           ...
           #TODO, add these to the structure? condor_job_ads - dict - condor
           related job information retry_count - int - generated field based
           on length of retry_ids retry_ids - list - list of jobs that are
           retried based off of this job retry_parent - str - job_id of the
           parent this retry is based off of. Not available on a retry_parent
           itself retry_saved_toggle - bool - Marked true when all retry
           steps/txns have completed parent_job_id - str - job_id taken from
           job_input.parent_job_id batch_job - bool - whether or not this is
           a batch parent container child_jobs - array - Only parent
           container should have child job ids scheduler_type - str -
           scheduler, such as awe or condor scheduler_id - str - scheduler
           generated id scheduler_estimator_id - str - id for the job spawned
           for estimation terminated_code - int - internal reason why a job
           was terminated, one of: 0 - user cancellation 1 - admin
           cancellation 2 - terminated by some automatic process @optional
           error @optional error_code @optional errormsg @optional
           terminated_code @optional estimating @optional running @optional
           finished) -> structure: parameter "job_id" of type "job_id" (A job
           id.), parameter "user" of String, parameter "authstrat" of String,
           parameter "wsid" of Long, parameter "status" of String, parameter
           "job_input" of type "RunJobParams" (method - the SDK method to run
           in module.method format, e.g. 'KBaseTrees.construct_species_tree'
           params - the parameters to pass to the method. Optional
           parameters: app_id - the id of the Narrative application (UI)
           running this job (e.g. repo/name) service_ver - specific version
           of deployed service, last version is used if this parameter is not
           defined source_ws_objects - denotes the workspace objects that
           will serve as a source of data when running the SDK method. These
           references will be added to the autogenerated provenance. Must be
           in UPA format (e.g. 6/90/4). meta - Narrative metadata to
           ...
           rather than owner of the job parent_job_id - EE2 job id for the
           parent of the current job. For run_job and run_job_concierge, this
           value can be specified to denote the parent job of the job being
           created. Warning: No checking is done on the validity of the job
           ID, and the parent job record is not altered. Submitting a job
           with a parent ID to run_job_batch will cause an error to be
           returned. job_requirements: the requirements for the job. The user
           must have full EE2 administration rights to use this parameter.
           Note that the job_requirements are not returned along with the
           rest of the job parameters when querying the EE2 API - they are
           only considered when submitting a job. as_admin: run the job with
           full EE2 permissions, meaning that any supplied workspace IDs are
           not checked for accessibility and job_requirements may be
           supplied. The user must have full EE2 administration rights. Note
           that this field is not included in returned data when querying
           EE2.) -> structure: parameter "method" of String, parameter
           "app_id" of String, parameter "params" of list of unspecified
           object, parameter "service_ver" of String, parameter
           "source_ws_objects" of list of type "wsref" (A workspace object
           reference of the form X/Y/Z, where X is the workspace id, Y is the
           object id, Z is the version.), parameter "meta" of type "Meta"
           (Narrative metadata for a job. All fields are optional. run_id -
           the Narrative-assigned ID of the job run. 1:1 with a job ID.
           token_id - the ID of the token used to run the method. tag - the
           release tag, e.g. dev/beta/release. cell_id - the ID of the
           narrative cell from which the job was run.) -> structure:
           parameter "run_id" of String, parameter "token_id" of String,
           parameter "tag" of String, parameter "cell_id" of String,
           parameter "wsid" of Long, parameter "parent_job_id" of String,
           parameter "job_requirements" of type "JobRequirements" (Job
           requirements for a job. All fields are optional. To submit job
           requirements, the user must have full EE2 admin permissions.
           Ignored for the run concierge endpoint. request_cpus: the number
           of CPUs to request for the job. request_memory: the amount of
           memory, in MB, to request for the job. request_disk: the amount of
           disk space, in GB, to request for the job. client_group: the name
           of the client group on which to run the job. client_group_regex:
           Whether to treat the client group string, whether provided here,
           from the catalog, or as a default, as a regular expression when
           matching clientgroups. Default True for HTC, but the default
           depends on the scheduler. Omit to use the default. bill_to_user:
           the job will be counted against the provided user's fair share
           quota. ignore_concurrency_limits: ignore any limits on
           simultaneous job runs. Default false. scheduler_requirements:
           arbitrary key-value pairs to be provided to the job scheduler.
           Requires knowledge of the scheduler interface. debug_mode: Whether
           to run the job in debug mode. Default false.) -> structure:
           parameter "request_cpus" of Long, parameter "requst_memory" of
           Long, parameter "request_disk" of Long, parameter "client_group"
           of String, parameter "client_group_regex" of type "boolean"
           (@range [0,1]), parameter "bill_to_user" of String, parameter
           "ignore_concurrency_limits" of type "boolean" (@range [0,1]),
           parameter "scheduler_requirements" of mapping from String to
           String, parameter "debug_mode" of type "boolean" (@range [0,1]),
           parameter "as_admin" of type "boolean" (@range [0,1]), parameter
           ...
           to the job (from the run_job call)  ## TODO - verify job_output -
           object - outputs from the job (from the run_job call) ## TODO -
