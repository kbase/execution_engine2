...
def test_job_req_init_maximal():
    jr = JobRequirements(
        6, 7, 8, "bigmemlong", True, 'someuser', True, {'proc': 'x286', 'maxmem': '640k'}
    )
...
def test_job_req_init_fail():
    n = None
    _job_req_init_fail(n, 1, 1, "f", n, n, IncorrectParamsException(
        'CPU count must be at least 1'))
    _job_req_init_fail(0, 1, 1, "f", n, n, IncorrectParamsException(
        'CPU count must be at least 1'))
    _job_req_init_fail(1, n, 1, "f", n, n, IncorrectParamsException(
        'memory in MB must be at least 1'))
    _job_req_init_fail(1, 0, 1, "f", n, n, IncorrectParamsException(
        'memory in MB must be at least 1'))
    _job_req_init_fail(1, 1, n, "f", n, n, IncorrectParamsException(
        'disk space in GB must be at least 1'))
    _job_req_init_fail(1, 1, 0, "f", n, n, IncorrectParamsException(
        'disk space in GB must be at least 1'))
    _job_req_init_fail(1, 1, 1, n, n, n, IncorrectParamsException(
        'Missing input parameter: client_group'))
    _job_req_init_fail(1, 1, 1, "  \t    ", n, n, IncorrectParamsException(
        'Missing input parameter: client_group'))
    # as_user is optional, so this is the only possible failure mode
    _job_req_init_fail(1, 1, 1, "f", "user\tname", n, IncorrectParamsException(
        'as_user contains control characters'))
    _job_req_init_fail(1, 1, 1, "f", n, {n: "a"}, IncorrectParamsException(
        'Missing input parameter: key in scheduler requirements structure'))
    _job_req_init_fail(1, 1, 1, "f", n, {"    \t   ": "a"}, IncorrectParamsException(
        'Missing input parameter: key in scheduler requirements structure'))
    _job_req_init_fail(1, 1, 1, "f", n, {"a": n}, IncorrectParamsException(
...
def test_equals():
    ...
    assert JobRequirements(1, 1, 1, c1, t, u1, f, r1) != (
        1, 1, 1, c1a, t, u1a, f, r1a)
...
def test_hash():
    ...
    assert hash(JobRequirements(1, 1, 1, c1, t, u1, f, r1)) == hash(JobRequirements(
        1, 1, 1, c1a, t, u1a, f, r1a))
