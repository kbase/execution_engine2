...
def test_parse_bool_failure():
    ...
    for tc in testcases:
        with raises(Exception) as e:
            parse_bool(tc)
        assert_exception_correct(e.value, IncorrectParamsException(f"{tc} is not a boolean value"))
...
def test_check_string_optional_false():
    for string in [None, '   \t   ']:
        ...
        assert_exception_correct(got.value, IncorrectParamsException(
            'Missing input parameter: var name'))
...
def test_check_string_long_fail():
    for string, length in {'123456789': 8,
        ...
        assert_exception_correct(
            got.value, IncorrectParamsException(f'var name exceeds maximum length of {length}'))
