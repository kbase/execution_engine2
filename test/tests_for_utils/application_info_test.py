...
def test_app_info_init_fail():
    m = "m.n"
    a = "m.b"
    _app_info_init_fail(None, a, False, IncorrectParamsException(
        'Missing input parameter: method ID'))
    _app_info_init_fail("   \t    ", a, False, IncorrectParamsException(
        'Missing input parameter: method ID'))
    _app_info_init_fail("   method   ", a, False, IncorrectParamsException(
        "Expected exactly one '.' in method ID 'method'"))
    _app_info_init_fail("   mod.innermod.method   ", a, False, IncorrectParamsException(
        "Expected exactly one '.' in method ID 'mod.innermod.method'"))
    _app_info_init_fail("    .  meth", a, False, IncorrectParamsException(
        'Missing input parameter: module portion of method ID'))
    _app_info_init_fail(" mod   .  ", a, False, IncorrectParamsException(
        'Missing input parameter: method portion of method ID'))

    _app_info_init_fail(m, None, False, IncorrectParamsException(
        'Missing input parameter: application ID'))
    _app_info_init_fail(m, "   \t    ", False, IncorrectParamsException(
        'Missing input parameter: application ID'))
    _app_info_init_fail(m, "mod / meth.bak ", False, IncorrectParamsException(
        "Application ID 'mod / meth.bak' has both '/' and '.' separators"))
    _app_info_init_fail(m, "mod / meth / bak ", False, IncorrectParamsException(
        "Expected exactly one '/' in application ID 'mod / meth / bak'"))
    _app_info_init_fail(m, "mod.meth", True, IncorrectParamsException(
        "Application ID 'mod.meth' contains a '.'"))
    _app_info_init_fail(m, "mod.meth.anothermeth", False, IncorrectParamsException(
        "Expected exactly one '.' in application ID 'mod.meth.anothermeth'"))

    _app_info_init_fail("mod.meth", "  mod2  /meth", True, IncorrectParamsException(
        "Application module 'mod2' must equal method module 'mod'"))

    _app_info_init_fail(m, "mod/", False, IncorrectParamsException(
        "Missing input parameter: application portion of application ID"))
    _app_info_init_fail(m, "/meth", False, IncorrectParamsException(
