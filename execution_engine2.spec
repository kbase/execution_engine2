    module execution_engine2 {
        ...
        */
        ...
        /*
            ##########################################################################
            #            Note about tags and semantic versions in narrative:         #
            ##########################################################################
            Code cells (Generated by app UIs) are submitted with
                tag - optional, one of [release|beta|dev] (default=release)
                version - e.g 1.1.0 or git commit

            The default UI for a "Released app" will generate a tag of "release" and populate the version string with
            a semantic version (such as "1.0.0")
            Removing the tag of "release" will cause the tag to default to release anyway.

            The default UI for a "Beta" or "Dev" app will generate a tag of "beta|dev" and populate the version string with
            a git commit hash.

            If submitting a code cell with a tag of beta or dev, a valueError will be raised if a semantic version is provided,
            (such as 1.1.0). However, you can submit a git commit hash instead.
